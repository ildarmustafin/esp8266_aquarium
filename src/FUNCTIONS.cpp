...
void led_schedule()
{
  if (strcmp(sched_led[ntp.week][0], "") || strcmp(sched_led[ntp.week][1], "") || strcmp(sched_led[ntp.week][2], "") || strcmp(sched_led[ntp.week][3], ""))
  {
    ...
    case 0:
      ...
      pwmController.setChannelPWM(LEDLED, 4096 - LED_value);
      break;
    case 1:
      state.led = 100;
      pwmController.setChannelOn(LEDPIN);
      pwmController.setChannelOff(LEDLED);
      break;
    case 2:
      state.led = 0;
      pwmController.setChannelOff(LEDPIN);
      pwmController.setChannelOn(LEDLED);
      break;
    }
  }
}
...
void relay_schedule()
{
  ...
  if (strcmp(sched_rel[ntp.week][0], "") || strcmp(sched_rel[ntp.week][1], "") || strcmp(sched_rel[ntp.week][2], "") || strcmp(sched_rel[ntp.week][3], ""))
  {
    ...
    pwmController.setChannelPWM(RELAY1LED, !PWMrel1 * 4096);
    pwmController.setChannelPWM(RELAY2LED, !PWMrel2 * 4096);
  }
}
void update_chart_values()
{
  ...
  if (!ostatok && !flag.chart)        //если 2 часа и chartFlag
  {
    ...
    jsonWrite(configChart, "days", rtc.now().day(), point, option.tempC); //1736 - 1786
    Serial.print(F("chart. size: "));
    Serial.println(configChart.length());
...
void temp_fan_regulation(float t)
{
  ...
  pwmController.setChannelPWM(FANLED, !state.fan * 4096);
}
void temp_ten_regulation(float t)
{
  ...
  pwmController.setChannelPWM(TENLED, !state.ten * 4096);
}
void initOTA()
{
